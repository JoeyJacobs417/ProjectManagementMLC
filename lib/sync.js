...
// Vult de bulk-cache voor één specifieke week (maandag → maandag+7). Klein genoeg om
// nooit te time-outen. Door dit 4× aan te roepen (handmatig vanaf de medewerker-pagina)
// bouw je de volledige 4-weken-cache op, inclusief uren op ongekoppelde Moneybird-projecten.
export async function warmMoneybirdWeek(weekStartIso) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(weekStartIso || ''))) {
    throw new Error('Ongeldige week_start (verwacht YYYY-MM-DD)');
  }
  ...
  // Alle entries (alle medewerkers, alle projecten) in [start, end) ophalen.
  const raw = await fetchAllTimeEntriesWithFilter(`started_after:${start},started_before:${end}`);
  const fresh = raw.map(normalizeTimeEntry);

  // Bestaande cache: behoud entries buiten dit venster (en binnen retention).
  const existing = await getRecentMoneybirdEntries();
  const kept = (existing && Array.isArray(existing.time_entries) ? existing.time_entries : [])
    .filter((e) => {
      if (!e.started_at) return false;
      const day = String(e.started_at).slice(0, 10);
      if (day < cutoffIso) return false;
  ...
  await setRecentMoneybirdEntries(payload);
  return { count: merged.length, fresh: fresh.length, week: start, since: effectiveSince };
}
