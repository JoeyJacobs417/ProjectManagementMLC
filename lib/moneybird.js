...
export async function fetchUsers() {
  // Lijst van gebruikers in deze administratie. Vereist de juiste API-token scope.
  return getJSON('/users.json');
}

export async function fetchTimeEntries(projectId) {
  const out = [];
  let page = 1;
  while (true) {
    const params = { page, per_page: 100 };
    if (projectId) params.filter = `project_id:${projectId}`;
