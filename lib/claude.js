...
// Default instructies. Kunnen overschreven worden per-upload via 'userPrompt' option
// (zie api/projects/parse-pdf.js) of via admin settings.pdf_prompt.
export const DEFAULT_PDF_PROMPT = `Lees de bijgevoegde PDF (een offerte of projectvoorstel) en haal de volgende informatie eruit:
...
export async function parseProjectPdf(buffer, options = {}) {
  ...
  // Custom prompt of default
  const userPromptText = (options.userPrompt && options.userPrompt.trim())
  ...
  console.log('Claude PDF parse start', {
    ...
    custom_prompt: userPromptText !== DEFAULT_PDF_PROMPT,
  });
