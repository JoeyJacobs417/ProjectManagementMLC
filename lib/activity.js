...
export function newActivityId() { return 'a_' + crypto.randomBytes(6).toString('hex'); }
...
function teamIdSet(team) { return new Set((team || []).map((t) => t.moneybird_user_id)); }
...
export function diffProjectActivity(oldP, newP, user) {
  ...
  // Modules diff (vergelijk arrays als sets)
  const oldMods = modulesOf(oldP).slice().sort().join(',');
  const newMods = modulesOf(newP).slice().sort().join(',');
  if (oldMods !== newMods) {
    out.push(entry(user, 'modules_changed', {
      from: modulesOf(oldP),
      to: modulesOf(newP),
    }));
  }
  ...
  if ((oldP.start_date || '') !== (newP.start_date || '')) {
    out.push(entry(user, 'start_date_changed', { from: oldP.start_date || '', to: newP.start_date || '' }));
  }
  if ((oldP.client_id || '') !== (newP.client_id || '')) {
    out.push(entry(user, 'client_changed', { from: oldP.client_id || '', to: newP.client_id || '' }));
  }
  if (!!oldP.is_poc !== !!newP.is_poc) {
    out.push(entry(user, 'poc_changed', { to: !!newP.is_poc }));
  }
  if ((oldP.feature_requests || '') !== (newP.feature_requests || '')) {
    out.push(entry(user, 'feature_requests_changed', {}));
  }
  ...
  for (const t of added) out.push(entry(user, 'team_added', { name: t.name, moneybird_user_id: t.moneybird_user_id }));
  for (const t of removed) out.push(entry(user, 'team_removed', { name: t.name, moneybird_user_id: t.moneybird_user_id }));
