...
export async function computeNextWeekPlanning() {
  ...
  const settings = await getSettings();
  const planningEmpIds = new Set(settings.dashboard_planning_employees || []);
  ...
  // Filter op geconfigureerde planning-medewerkers (indien ingesteld).
  const empList = planningEmpIds.size > 0
    ? Array.from(byEmp.values()).filter((e) => planningEmpIds.has(e.id))
    : Array.from(byEmp.values());
