...
export async function computeNextWeekPlanning() {
  ...
  const settings = await getSettings();
  const planningEmpIds = settings.dashboard_planning_employees || [];

  // Haal alle interne gebruikers op; filter op geconfigureerde planning-medewerkers.
  const allUsers = await listUsers();
  const empUsers = planningEmpIds.length > 0
    ? allUsers.filter((u) => planningEmpIds.includes(u.moneybird_user_id || u.id))
    : allUsers;

  const projects = await listProjects();

  // Bouw per medewerker een lijst van ingeplande projecten voor de komende week.
  const empPlanning = empUsers.map((u) => {
    const uid = u.moneybird_user_id || u.id;
    const tasks = [];
    for (const p of projects) {
      if (p.status === 'done') continue;
      const alloc = (p.weekly_allocations || []).find(
        (a) => a.moneybird_user_id === uid && a.week_start === weekIso
      );
      if (alloc && Number(alloc.hours) > 0) {
        tasks.push({ projectName: p.name, hours: Number(alloc.hours) });
      }
    }
    return { name: u.name || u.email || uid, tasks };
  });
