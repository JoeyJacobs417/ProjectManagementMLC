// Drempel-controle + inactiviteits-alert + periodieke rapportage + e-mail via Resend.
...
function buildReportHtml(items, period) {
  ...
  const rows = items.map(({ project: p, stats }) => `
    <tr>
      <td style="padding:8px;border-bottom:1px solid #e5e7eb"><strong>${escapeHtml(p.name)}</strong>${p.module ? ` <span style="color:#64748b">— ${escapeHtml(p.module)}</span>` : ''}</td>
