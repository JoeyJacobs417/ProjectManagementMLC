...
export const DEFAULT_TEMPLATES = {
  threshold_warning: {
    subject: '[Projectmanager] Update — {project_name} op {percentage}%',
    body: `<h2>Update: {project_name} zit op {percentage}% van de uren</h2>
<p>{greeting},</p>
...
beschikbare uren verbruikt (<strong>{percentage}%</strong>).</p>`,
  ...
  threshold_exceeded: {
    subject: '[Projectmanager] Update — {project_name}',
    body: `<h2>Update: {project_name}</h2>
<p>{greeting},</p>
<p>Het project <strong>{project_name}</strong> heeft op dit moment
<strong>{hours_used}</strong> van de <strong>{available_hours}</strong>
beschikbare uren verbruikt (<strong>{percentage}%</strong>).</p>`,
...
export const DEFAULT_TEMPLATES_EN = {
  threshold_warning: {
    subject: '[Projectmanager] Update — {project_name} at {percentage}%',
    body: `<h2>Update: {project_name} is at {percentage}% of its hours</h2>
<p>{greeting},</p>
...
// Verzamelt alle deadlines van een project: nieuwe deadlines-array of legacy enkel veld.
function projectDeadlines(project) {
  if (Array.isArray(project.deadlines) && project.deadlines.length > 0) {
    return project.deadlines
      .filter((d) => d && d.date)
      .map((d) => ({ date: String(d.date), description: String(d.description || '') }));
  }
  if (project.deadline) return [{ date: String(project.deadline), description: '' }];
  return [];
}

export async function checkDeadlineAlertForProject(project) {
  if (project.status === 'done') return null;
  const deadlines = projectDeadlines(project);
  ...
  for (const d of deadlines) {
    ...
    // Dedup-key per deadline-datum, zodat elke deadline zijn eigen alert krijgt.
    const descSuffix = d.description ? ` — ${d.description}` : '';

    if (days < 0) {
      if (await isDeadlineAlertSent(project.id, d.date, 'passed')) continue;
      const vars = {
        project_name: escapeHtml(project.name),
        deadline_date: fmtDate(d.date),
        deadline_description: escapeHtml(d.description || ''),
        days_overdue: -days,
      };
      const okCount = await sendPersonalized(recipients, settings, 'deadline_passed', vars);
      if (okCount === 0) continue;
      await recordDeadlineAlert(project.id, d.date, 'passed');
      results.push('passed' + descSuffix);
    } else if (days <= threshold) {
      if (await isDeadlineAlertSent(project.id, d.date, 'approaching')) continue;
      const vars = {
        project_name: escapeHtml(project.name),
        deadline_date: fmtDate(d.date),
        deadline_description: escapeHtml(d.description || ''),
      ...
      if (okCount === 0) continue;
      await recordDeadlineAlert(project.id, d.date, 'approaching');
      results.push('approaching' + descSuffix);
    }
  }
...
function sentimentCellHtml(v) {
  const map = { green: ['#047857', 'Groen'], orange: ['#b45309', 'Oranje'], red: ['#b91c1c', 'Rood'] };
  const m = map[v];
  if (!m) return '<span style="color:#94a3b8">—</span>';
  return `<span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${m[0]};vertical-align:middle"></span> <span style="color:${m[0]};font-weight:600">${m[1]}</span>`;
}
...
export async function computeMonthForecast() {
  ...
  // Handmatig toegevoegde omzet (pagina Omzet) voor deze maand — geen uren, alleen omzet.
  for (const m of (settings.manual_revenue || [])) {
    if (m.month !== monthIso) continue;
    const amount = Number(m.amount) || 0;
    if (amount <= 0) continue;
    const rec = ensureEmp(String(m.moneybird_user_id), m.employee_name);
    rec.realizedRevenue += amount;
    rec.manualPosts.push({ description: m.description || '', amount });
  }
  ...
  // Bezetting: ruwe ingeplande uren / (capaciteit × weken) — niet budget-gecapped
  // Telt project-allocaties + overig + vakantiedagen op, net als "Totaal per week" op de planningspagina.
  const weekSet = new Set(weeks);
  const overigByEmp = settings.employee_overig || {};
  const rawHoursByEmp = new Map();
  // 1. Project-allocaties
  for (const p of enriched) {
    if (p.status === 'done') continue;
    for (const a of (p.weekly_allocations || [])) {
      if (!weekSet.has(a.week_start)) continue;
      const id = a.moneybird_user_id;
      rawHoursByEmp.set(id, (rawHoursByEmp.get(id) || 0) + (Number(a.hours) || 0));
    }
  }
  // 2. Overig-uren
  for (const [empId, empWeeks] of Object.entries(overigByEmp)) {
    for (const [weekStart, entry] of Object.entries(empWeeks)) {
      if (!weekSet.has(weekStart)) continue;
      rawHoursByEmp.set(empId, (rawHoursByEmp.get(empId) || 0) + (Number(entry?.hours) || 0));
    }
  }
  // 3. Vakantiedagen (capacity/5 per dag)
  for (const [empId, periods] of Object.entries(vacations)) {
    const cap = Number(capacities[empId]) || 0;
    if (!cap) continue;
    const hoursPerDay = cap / 5;
    for (const weekStart of weeks) {
      const [wy, wm, wd] = weekStart.split('-').map(Number);
      for (let i = 0; i < 5; i++) {
        const dd = new Date(wy, wm - 1, wd + i);
        const iso = toIso(dd);
        if (periods.some((p) => p.start <= iso && iso <= p.end)) {
          rawHoursByEmp.set(empId, (rawHoursByEmp.get(empId) || 0) + hoursPerDay);
        }
      }
    }
  }
  ...
  const rows = Array.from(empMap.values())
    .map((r) => {
      const raw = rawHoursByEmp.get(r.id) || 0;
      const cap = Number(capacities[r.id]) || 0;
      const totalCap = cap * weeks.length;
      const bezetting = (cap > 0 && weeks.length > 0)
        ? Math.round((raw / totalCap) * 100)
        : null;
      return { ...r, forecastRevenue: r.realizedRevenue + r.plannedRevenue, bezetting, rawHours: raw, totalCap };
    })
...
// Berekent de maandag van de volgende kalenderweek (UTC).
function nextWeekMonday() {
  const today = new Date(); today.setUTCHours(0, 0, 0, 0);
  const dow = today.getUTCDay(); // 0=zo … 6=za
...
export async function computeNextWeekPlanning() {
  ...
  // Werkdagen van de komende week (ma t/m vr).
  const weekdays = [];
  for (let d = 0; d < 5; d++) {
    const dd = new Date(monday); dd.setUTCDate(dd.getUTCDate() + d);
    weekdays.push(toIso(dd));
  }

  // Bouw naam-lookup op vanuit alle beschikbare bronnen in de KV (geen live API-call nodig).
  const nameById = new Map();
  // 1. Project-team entries
  for (const p of projects) {
    for (const t of (p.team || [])) {
      const id = String(t.moneybird_user_id || t.id || '').trim();
      if (id && t.name) nameById.set(id, String(t.name).trim());
    }
  }
  // 2. Gecachede tijdregistraties — zelfde bron als computeMonthForecast
  try {
    const recent = await getRecentMoneybirdEntries();
    for (const e of (recent?.time_entries || [])) {
      const id = String(e.user_moneybird_id || '').trim();
      if (id && e.user_name && !nameById.has(id)) nameById.set(id, String(e.user_name).trim());
    }
  } catch {}
  // 3. Moneybird-gebruikerscache (best-effort)
  const mbUsers = await getMoneybirdUserLookup();
  for (const [id, info] of mbUsers) {
    if (!nameById.has(id) && info.name) nameById.set(id, info.name);
  }

  // Bouw medewerkerlijst op vanuit project-team entries.
  const byEmp = new Map();
  for (const p of projects) {
    if (p.status === 'done') continue;
    for (const t of (p.team || [])) {
      const key = String(t.moneybird_user_id || t.id || '').trim();
      if (!key) continue;
      if (!byEmp.has(key)) byEmp.set(key, { id: key, name: t.name || nameById.get(key) || '' });
    }
  }

  // Voeg medewerkers toe via weekly_allocations op ALLE weken — niet alleen weekIso.
  // Zo worden medewerkers gevonden die op een project staan maar geen geldige team-entry hebben.
  // (Spiegelt hoe computeMonthForecast medewerkers ontdekt via allocaties.)
  for (const p of projects) {
    if (p.status === 'done') continue;
    for (const a of (p.weekly_allocations || [])) {
      const empId = String(a.moneybird_user_id || '').trim();
      if (!empId || byEmp.has(empId)) continue;
      const name = nameById.get(empId);
      if (name) byEmp.set(empId, { id: empId, name });
    }
  }

  // Voeg medewerkers toe die alleen vakantie of overig hebben maar niet via project gevonden zijn.
  for (const [empId, periods] of Object.entries(vacationsByEmp)) {
    if (byEmp.has(empId)) continue;
    const hasVac = weekdays.some((day) => periods.some((p) => p.start <= day && day <= p.end));
    if (!hasVac) continue;
    const name = nameById.get(empId);
    if (name) byEmp.set(empId, { id: empId, name });
  }
  for (const [empId, weeks] of Object.entries(overigByEmp)) {
    if (byEmp.has(empId)) continue;
    const entry = weeks[weekIso];
    if (!entry || !Number(entry.hours)) continue;
    const name = nameById.get(empId);
    if (name) byEmp.set(empId, { id: empId, name });
  }

  // Opt-in: alleen medewerkers die via de medewerkers-pagina zijn aangevinkt voor planning.
  const planningSet = new Set(settings.planning_employees || []);
  const empList = Array.from(byEmp.values()).filter((e) => planningSet.has(e.id));

  // Sorteer op naam.
  empList.sort((a, b) => a.name.localeCompare(b.name));

  // Bouw per medewerker een lijst van werkzaamheden voor de komende week.
  const empPlanning = empList.map((emp) => {
    const tasks = [];

    // Project-allocaties
    for (const p of projects) {
      if (p.status === 'done') continue;
      const alloc = (p.weekly_allocations || []).find(
        (a) => a.moneybird_user_id === emp.id && a.week_start === weekIso
      );
      if (alloc && Number(alloc.hours) > 0) {
        tasks.push({ projectName: p.name, hours: Number(alloc.hours) });
      }
    }

    // Overig (losse taakjes) voor deze week
    const overigEntry = (overigByEmp[emp.id] || {})[weekIso];
    if (overigEntry && Number(overigEntry.hours) > 0) {
      const label = overigEntry.note ? `Overig — ${overigEntry.note}` : 'Overig';
      tasks.push({ projectName: label, hours: Number(overigEntry.hours), isOverig: true });
    }

    // Vakantie deze week
    const empVacations = vacationsByEmp[emp.id] || [];
    const vacDays = weekdays.filter((day) => empVacations.some((p) => p.start <= day && day <= p.end)).length;
    if (vacDays > 0) {
      const label = vacDays === 5 ? 'Vakantie (hele week)' : `Vakantie (${vacDays} ${vacDays === 1 ? 'dag' : 'dagen'})`;
      tasks.push({ projectName: label, hours: null, isVacation: true });
    }

    return { name: emp.name, tasks };
  });

  return { empPlanning, weekIso, fridayIso };
}

function buildNextWeekPlanningSectionHtml({ empPlanning, weekIso, fridayIso }) {
  ...
  const tableRows = empPlanning.map((emp) => {
    const tasksHtml = emp.tasks.length > 0
      ? emp.tasks.map((t) => {
          if (t.isVacation) return `<span style="color:#d97706">${escapeHtml(t.projectName)}</span>`;
          if (t.isOverig) return `<span style="color:#6366f1">${escapeHtml(t.projectName)}</span> <span style="color:#64748b">(${t.hours}u)</span>`;
          return `${escapeHtml(t.projectName)} <span style="color:#64748b">(${t.hours}u)</span>`;
        }).join('<br>')
      : '<span style="color:#94a3b8;font-style:italic">Geen werkzaamheden gepland</span>';
    const totalHours = emp.tasks.reduce((a, t) => a + (t.hours || 0), 0);
    const hoursHtml = totalHours > 0
      ? `<strong>${totalHours}u</strong>`
      : '<span style="color:#94a3b8">—</span>';
    return `<tr>
      <td style="${cellStyle}"><strong>${escapeHtml(emp.name)}</strong></td>
      <td style="${cellStyle}">${tasksHtml}</td>
      <td style="${cellStyle};text-align:right">${hoursHtml}</td>
    </tr>`;
  }).join('');

  return `<h2 style="color:#0f172a;margin:0 0 8px">Planning komende week</h2>
    <div style="color:#64748b;margin-bottom:8px">${weekIso} t/m ${fridayIso}</div>
    <table style="width:100%;border-collapse:collapse;background:#fff;border:1px solid #e5e7eb;margin-bottom:24px">
      <thead><tr style="background:#f7f8fc">
        ...
        <th style="${thStyle};text-align:right">Totaal uren</th>
      </tr></thead>
...
function buildForecastSectionHtml({ rows, grandTotal, target, monthIso }) {
  ...
  if (target > 0) {
    ...
    const pct = target > 0 ? Math.round(((grandTotal - target) / target) * 100) : 0;
    const onTarget = grandTotal >= target;
    const color = onTarget ? '#047857' : '#b91c1c';
    const icon = onTarget ? '✓' : '⚠';
    const pctLabel = onTarget ? `+${pct}%` : `${pct}%`;
    ...
    targetHtml = `<div style="margin:8px 0 12px;color:${color};font-weight:600">${icon} ${pctLabel} — ${label}</div>`;
  }
  if (rows.length === 0) {
  ...
  const tableRows = rows.map((r) => {
    const bezettingColor = r.bezetting === null ? '' : r.bezetting > 100 ? '#b91c1c' : r.bezetting >= 80 ? '#047857' : '#b45309';
    const bezettingHtml = r.bezetting !== null
      ? `<strong style="color:${bezettingColor}">${r.bezetting}%</strong>`
      : '<span style="color:#94a3b8">—</span>';
    return `
    <tr>
      <td style="${cellStyle}"><strong>${escapeHtml(r.name)}</strong></td>
      <td style="${cellStyle}">${r.realizedRevenue > 0
        ? `<strong>${fmtEuro(r.realizedRevenue)}</strong>${r.realizedHours > 0 ? ` <span style="color:#64748b">(${r.realizedHours.toFixed(1)} u)</span>` : ''}${(r.manualPosts && r.manualPosts.length)
            ? `<div style="color:#64748b;font-size:12px;margin-top:2px">${r.manualPosts.map((p) => `• ${escapeHtml(p.description || 'Onbekende taak')} — ${fmtEuro(p.amount)}`).join('<br>')}</div>`
            : ''}`
      ...
      <td style="${cellStyle}">${r.plannedRevenue > 0
        ...
        : '<span style="color:#64748b">—</span>'}</td>
      <td style="${cellStyle}">${bezettingHtml}</td>
      <td style="${cellStyle}"><strong>${fmtEuro(r.forecastRevenue)}</strong></td>
    </tr>`;
  }).join('');
  const totRealized = rows.reduce((a, r) => a + r.realizedRevenue, 0);
  const totRealizedH = rows.reduce((a, r) => a + r.realizedHours, 0);
  const totPotential = rows.reduce((a, r) => a + r.potentialRevenue, 0);
  const totPlanned = rows.reduce((a, r) => a + r.plannedRevenue, 0);
  const totPlannedH = rows.reduce((a, r) => a + r.plannedHours, 0);
  const totRaw = rows.reduce((a, r) => a + (r.rawHours || 0), 0);
  const totCap = rows.reduce((a, r) => a + (r.totalCap || 0), 0);
  const totBez = totCap > 0 ? Math.round((totRaw / totCap) * 100) : null;
  const totBezColor = totBez === null ? '' : totBez > 100 ? '#b91c1c' : totBez >= 80 ? '#047857' : '#b45309';
  const tfStyle = `padding:8px;border-top:2px solid #e5e7eb;background:#f7f8fc;font-size:13px`;
  const totalsRow = `<tr>
    <td style="${tfStyle}"><strong>Totaal</strong></td>
    <td style="${tfStyle}">${totRealized > 0 ? `<strong>${fmtEuro(totRealized)}</strong> <span style="color:#64748b">(${totRealizedH.toFixed(1)} u)</span>` : '<span style="color:#64748b">—</span>'}</td>
    <td style="${tfStyle}">${totPotential > 0 ? `<strong>${fmtEuro(totPotential)}</strong>` : '<span style="color:#64748b">—</span>'}</td>
    <td style="${tfStyle}">${totPlanned > 0 ? `<strong>${fmtEuro(totPlanned)}</strong> <span style="color:#64748b">(${totPlannedH.toFixed(1)} u)</span>` : '<span style="color:#64748b">—</span>'}</td>
    <td style="${tfStyle}">${totBez !== null ? `<strong style="color:${totBezColor}">${totBez}%</strong>` : '<span style="color:#64748b">—</span>'}</td>
    <td style="${tfStyle}"><strong>${fmtEuro(grandTotal)}</strong></td>
  </tr>`;
...
function buildProjectsSectionHtml(items) {
  const rows = items.map(({ project: p, stats, manager_name }) => `
    <tr>
      ...
      <td style="padding:8px;border-bottom:1px solid #e5e7eb">${sentimentCellHtml(p.sentiment)}</td>
      <td style="padding:8px;border-bottom:1px solid #e5e7eb">${manager_name ? escapeHtml(manager_name) : '—'}</td>
  ...
  return `<h2 style="color:#0f172a;margin:0 0 8px">Status projecten</h2>
    ...
    <table style="width:100%;border-collapse:collapse;background:#fff;border:1px solid #e5e7eb">
      <thead><tr style="background:#f7f8fc">
        ...
        <th style="${thStyle}">Projectmanager</th>
...
function buildReportHtml(items, period, forecast, nextWeekPlanning) {
  ...
  // Sectie weglaten als er geen medewerkers voor planning zijn aangevinkt.
  const planningHtml = (nextWeekPlanning && nextWeekPlanning.empPlanning.length > 0)
    ? buildNextWeekPlanningSectionHtml(nextWeekPlanning) : '';
...
export async function sendProjectReport({ force = false } = {}) {
  ...
  for (const p of projects) {
    ...
    const manager = p.manager_id ? await getUserById(p.manager_id) : null;
    items.push({ project: p, stats, manager_name: manager?.name || '' });
  }
  ...
  let nextWeekPlanning = null;
  try { nextWeekPlanning = await computeNextWeekPlanning(); }
  catch (err) { console.error('Planning voor rapportage faalde:', err.message); }
  try {
    await setup.client.emails.send({ from: setup.from, to: emails, subject, html: buildReportHtml(items, period, forecast, nextWeekPlanning) });
  } catch (err) {
...
// Bepaalt het meest recente geplande verzendmoment (<= nu, UTC) als YYYY-MM-DD,
// of null als rapportage uit staat. Hierop is de "inhaal"-logica gebaseerd: als een
// cron-run gemist of getimed-out is, stuurt de eerstvolgende run de mail alsnog.
function scheduledDueDateIso(settings, now) {
  if (settings.report_period === 'weekly') {
    const wantDow = Number.isInteger(settings.report_day_of_week) ? settings.report_day_of_week : 1;
    const daysBack = (now.getUTCDay() - wantDow + 7) % 7;
    const d = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() - daysBack));
    return toIso(d);
  }
  if (settings.report_period === 'monthly') {
    ...
    const clampDom = (yy, mm) => Math.min(wantDom, new Date(Date.UTC(yy, mm + 1, 0)).getUTCDate());
    const thisMonthDue = clampDom(y, m);
    if (now.getUTCDate() >= thisMonthDue) return toIso(new Date(Date.UTC(y, m, thisMonthDue)));
    const py = m === 0 ? y - 1 : y;
    const pm = m === 0 ? 11 : m - 1;
    return toIso(new Date(Date.UTC(py, pm, clampDom(py, pm))));
  }
  return null;
}
...
export async function maybeSendScheduledReport(now = new Date()) {
  ...
  const dueIso = scheduledDueDateIso(settings, now);
  if (!dueIso) return { sent: false, reason: 'period_off' };

  // Al verstuurd op of na dit geplande moment? Dan klaar — voorkomt dubbel versturen
  // en zorgt dat een ingehaalde mail niet nogmaals gaat.
  const lastSent = await getLastReportSentDate();
  if (lastSent && lastSent >= dueIso) return { sent: false, reason: 'already_sent' };

  // Uur-afdwinging (report_hour, 0–23 UTC) geldt alléén op de geplande dag zelf.
  // Loopt de verzending achter door een gemiste/getimed-oute run, dan sturen we bij de
  // eerstvolgende run alsnog — ongeacht het uur. Leeg = geen afdwinging.
  const isDueDay = toIso(now) === dueIso;
  if (isDueDay && Number.isInteger(settings.report_hour) && now.getUTCHours() < settings.report_hour) {
    return { sent: false, reason: 'waiting_for_hour' };
  }
