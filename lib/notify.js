...
// ── Default mailteksten (gebruiken nu {greeting} voor personalisatie) ──
...
// ── Recipients met naam (voor personalisatie) ───────────────────────
// Returns: [{ email, name, firstName }]
async function recipientsForPersonalized(project, settings, { includeOptedInContacts = false, includeTeam = false } = {}) {
  ...
  function add(email, name) {
    ...
    seen.add(e);
    out.push({ email: String(email).trim(), name: name || '', firstName: firstNameOf(name) });
  }
  ...
  // Extra ontvangers (komma-string, zonder naam)
  ...
  // Klantcontacten met opt-in (voor threshold-mails)
  if (includeOptedInContacts && Array.isArray(project.contacts)) {
    for (const c of project.contacts) {
      if (c && c.receives_threshold_mails && c.email) add(c.email, c.name || '');
    }
  }
  return out;
}
...
// Verstuurt per ontvanger een eigen mail met gepersonaliseerde body.
// Returns aantal succesvol verstuurde mails.
async function sendPersonalized(recipients, subjectTpl, bodyTpl, vars) {
  ...
  for (const r of recipients) {
    ...
    const subject = renderTemplate(subjectTpl, personalVars);
    const html = renderTemplate(bodyTpl, personalVars);
    try {
...
export async function checkAndNotify(project) {
  ...
  const tpl = templateFor(settings, `threshold_${level}`);
  const vars = {
  ...
  const sent = await sendPersonalized(recipients, tpl.subject, tpl.body, vars);
...
export async function checkInactivityAlerts(project, oldEntries, newEntries) {
  ...
  for (const evt of events.values()) {
    ...
    const okCount = await sendPersonalized(recipients, tpl.subject, tpl.body, vars);
...
export async function sendProjectReport({ force = false } = {}) {
  ...
  try {
    await setup.client.emails.send({ from: setup.from, to: emails, subject, html: buildReportHtml(items, period) });
  } catch (err) {
