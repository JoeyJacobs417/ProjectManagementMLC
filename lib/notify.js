...
// ── Inactiviteits-mails ──────────────────────────────────────────────
// Vergelijkt oude vs. nieuwe entries voor een project. Stuurt een mail
// als een persoon weer uren schrijft na een pauze > settings.inactivity_days.
...
export async function checkInactivityAlerts(project, oldEntries, newEntries) {
  ...
  // Per (user, returning_date) eerst aggregeren — dan max 1 mail per dag per persoon
  const events = new Map(); // key: user_moneybird_id|date
  for (const e of fresh) {
    if (!e.user_moneybird_id || !e.started_at) continue;
    const lastIso = lastByUser.get(e.user_moneybird_id);
    if (!lastIso) continue; // nooit eerder op dit project gewerkt -> geen "terugkeer"
