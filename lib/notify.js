...
export async function computeNextWeekPlanning() {
  ...
  for (const p of projects) {
    ...
    for (const t of (p.team || [])) {
      ...
      if (!key) continue;
      if (!byEmp.has(key)) byEmp.set(key, { id: key, name: t.name });
    }
  }

  // Voeg medewerkers toe die alleen vakantie of overig hebben deze week maar niet in een actief project zitten.
  const mbUsers = await getMoneybirdUserLookup();
  for (const [empId, periods] of Object.entries(vacationsByEmp)) {
    ...
    if (!hasVac) continue;
    const info = mbUsers.get(empId);
    if (info?.name) byEmp.set(empId, { id: empId, name: info.name });
  }
  ...
  // ── DEBUG (tijdelijk) ──────────────────────────────────────────────
  console.log('[planning-debug] weekIso:', weekIso);
  console.log('[planning-debug] byEmp na projectscan:', JSON.stringify(Array.from(byEmp.entries()).map(([k,v]) => ({ key: k, keyType: typeof k, name: v.name }))));
  console.log('[planning-debug] overigByEmp keys:', JSON.stringify(Object.keys(overigByEmp)));
  console.log('[planning-debug] overigByEmp entries voor weekIso:', JSON.stringify(Object.entries(overigByEmp).map(([id, weeks]) => ({ id, hasWeek: !!weeks[weekIso], hours: weeks[weekIso]?.hours }))));
  for (const p of projects) {
    if (p.status === 'done') continue;
    console.log(`[planning-debug] project "${p.name}" status=${p.status} team=${JSON.stringify((p.team||[]).map(t=>({ mb_id: t.moneybird_user_id, id: t.id, name: t.name })))}`);
  }
  // ── END DEBUG ─────────────────────────────────────────────────────
