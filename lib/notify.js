...
export async function computeNextWeekPlanning() {
  ...
  // Voeg medewerkers toe die een weekly_allocation hebben voor de komende week maar niet via p.team gevonden zijn.
  // (Spiegelt hoe computeMonthForecast medewerkers ontdekt via allocaties i.p.v. alleen teamlijst.)
  const mbUsers = await getMoneybirdUserLookup();
  for (const p of projects) {
    if (p.status === 'done') continue;
    for (const a of (p.weekly_allocations || [])) {
      if (a.week_start !== weekIso) continue;
