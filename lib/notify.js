...
export async function computeNextWeekPlanning() {
  ...
  // Bouw medewerkerlijst op vanuit project-team entries (zelfde aanpak als planning.html).
  // e.id = moneybird_user_id, wat ook in weekly_allocations en dashboard_planning_employees staat.
  const byEmp = new Map();
  for (const p of projects) {
    if (p.status === 'done') continue;
  ...
  // Bouw per medewerker een lijst van ingeplande projecten voor de komende week.
...
function buildNextWeekPlanningSectionHtml({ empPlanning, weekIso, fridayIso }) {
  ...
  const tableRows = empPlanning.map((emp) => {
    const tasksHtml = emp.tasks.length > 0
      ? emp.tasks.map((t) => t.isOverig
          ? `<span style="color:#6366f1">${escapeHtml(t.projectName)}</span> <span style="color:#64748b">(${t.hours}u)</span>`
          : `${escapeHtml(t.projectName)} <span style="color:#64748b">(${t.hours}u)</span>`
        ).join('<br>')
      : '<span style="color:#94a3b8;font-style:italic">Geen werkzaamheden gepland</span>';
    const totalHours = emp.tasks.reduce((a, t) => a + t.hours, 0);
