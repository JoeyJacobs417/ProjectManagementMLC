...
// Pre-warmed bulk-cache van recente Moneybird time entries (gevuld door dagelijkse cron).
// Bevat alle entries vanaf `since` (~35 dagen) zodat planning-pagina niet live hoeft te fetchen.
...
export async function setRecentMoneybirdEntries(payload) {
  // Geen TTL: blijft staan tot volgende cron-overschrijving (fallback voor zeldzame faal-runs).
...
const DEFAULT_SETTINGS = {
  ...
  clients: [],
  mail_templates: {},
  hidden_employees: [], // moneybird_user_ids die niet in de Medewerkers-tabel getoond worden
};
