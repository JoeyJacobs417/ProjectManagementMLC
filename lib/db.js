...
const DEFAULT_SETTINGS = {
  ...
  inactivity_days: 30,
  notify_emails_extra: '',
  pdf_prompt: '',                 // optioneel — leeg = default in lib/claude.js
  report_period: 'off',           // 'off' | 'weekly' | 'monthly'
  report_recipients: '',          // comma-separated e-mailadressen
  employee_capacities: {},        // moneybird_user_id -> hours per week
};

export async function getSettings() {
  const v = await kv.get(KEYS.settings);
  const merged = { ...DEFAULT_SETTINGS, ...(v || {}) };
  // Zorg dat het employee_capacities object er altijd is
...
// ───── Report dedupe (geen dubbele wekelijkse/maandelijkse mails op zelfde dag) ──
