...
// Verwijdert project + bijbehorende time-entries, PDF en alert-records.
export async function deleteProject(id) {
  const all = await listProjects();
  const filtered = all.filter((p) => p.id !== id);
  if (filtered.length === all.length) return false; // niets verwijderd
  await writeArr(KEYS.projects, filtered);

  // Opruimen van geassocieerde keys
...
// Eén pipelined Redis-roundtrip voor meerdere time_entries-keys ineens.
// Geeft een Map terug: projectId -> array.
export async function listTimeEntriesBatch(projectIds) {
  ...
  try {
    ...
    // Fallback bij pipeline-issues: sequentieel
...
// ───── Moneybird time-entries cache ─────────────────────────────────
const MB_CACHE_TTL = 600; // 10 minuten
export async function getCachedMoneybirdTimeEntries(since, userId) {
  return kv.get(KEYS.cacheMbTimeEntries(since, userId));
}
export async function setCachedMoneybirdTimeEntries(since, userId, payload) {
  await kv.set(KEYS.cacheMbTimeEntries(since, userId), payload, { ex: MB_CACHE_TTL });
}
export async function clearCachedMoneybirdTimeEntries() {
  // Helper voor manual refresh — we kunnen geen "del by pattern" doen zonder scan,
  // dus laten we cache gewoon op TTL verlopen. Functie geeft alleen "ok" terug.
  return true;
}
