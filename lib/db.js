...
export async function getPdfBlob(projectId) {
  return kv.get(KEYS.pdfBlob(projectId));
}

export async function deletePdfBlob(projectId) {
  await kv.del(KEYS.pdfBlob(projectId));
}
...
const DEFAULT_SETTINGS = {
  ...
  inactivity_days: 30,
  deadline_alert_days: 14,           // hoeveel dagen voor deadline waarschuwen
  notify_emails_extra: '',
  pdf_prompt: '',
  report_period: 'off',
  report_recipients: '',
