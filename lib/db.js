...
// ───── Moneybird caches ──────────────────────────────────────────────
const MB_TIME_ENTRIES_TTL = 600; // 10 min
const MB_USERS_TTL = 3600;       // 1 uur
...
export async function getCachedMoneybirdUsers() {
  return kv.get(KEYS.cacheMbUsers);
}
export async function setCachedMoneybirdUsers(users) {
  await kv.set(KEYS.cacheMbUsers, users, { ex: MB_USERS_TTL });
}
